...
class TemperatureMachine(StateMachine):
    ...
    def setupSerialOutput(self):
        ##
        ## FIXED
        output = 'State:' + self.current_state.id + f',Temp:{floor(self.getFahrenheit())}' + f',setPoint:{self.setPoint}'
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ##
                ## FIXED.
                lcd_line_2 = f'Temp:{floor(self.getFahrenheit())}'
    
                altCounter = altCounter + 1
            else:
                ##
                ## FIXED
                lcd_line_2 = f'State:{self.current_state.id}|{self.setPoint}'
            ...
            if((counter % 30) == 0):
                ...
                ## Commented out for now because my TTL-USB for the UART is faulty.                
                ## ser.write(self.setupSerialOutput().encode())

                ## printing to console from here to simulate what the server woudl display
                print('\n\nSERVER ouput would be: ' + self.setupSerialOutput() + '\n\n')
                counter = 1
            else:
                counter = counter + 1
            sleep(1)
